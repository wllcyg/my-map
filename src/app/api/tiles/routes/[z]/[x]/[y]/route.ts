...
import { routesTileCache } from "@/lib/mvtCache";
...
export async function GET(
  ...
  try {
    ...
    const cachedBuffer = routesTileCache.get(cacheKey);
    if (cachedBuffer) {
    ...
    const url = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/rpc/get_routes_mvt`;
    ...
    if (!response.ok) {
      ...
      console.error("[MVT Error] Routes API responded with:", response.status, errorText);
    ...
    if (buffer.length === 0) {
      routesTileCache.set(cacheKey, buffer);
      return new NextResponse(null, { status: 204 });
    }
    ...
    console.error("[MVT Error] Routes Tile route:", error.message);
